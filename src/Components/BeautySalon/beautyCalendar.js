...
function CalendarSlotSelector() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const automotiveCollection = collection(db, 'BeautySalon');
    ...
    const handleConfirmBooking = async () => {
        if (selectedSlot && documentId && !isSlotAlreadyBooked) {
            // Document references
            const bookingDocRef = doc(db, 'BeautySalon', doc1, 'booking', documentId);
            const companyDocRef = doc(db, 'BeautySalon', doc1);
