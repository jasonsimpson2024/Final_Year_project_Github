...
function CalendarSlotSelector() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const automotiveCollection = collection(db, 'Automotive'); // Adjust this to your collection name
    ...
    const handleConfirmBooking = async () => {
        if (selectedSlot && documentId && !isSlotAlreadyBooked) {
            const bookedDoc = doc(db, 'Automotive', doc1, 'booking', documentId); // Adjust path as necessary
