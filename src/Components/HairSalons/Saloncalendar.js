...
function CalendarSlotSelector() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const automotiveCollection = collection(db, 'HairSalon'); // Adjust this to your collection name
    ...
    const handleConfirmBooking = async () => {
        if (selectedSlot && documentId && !isSlotAlreadyBooked) {
            const bookedDoc = doc(db, 'HairSalon', doc1, 'booking', documentId); // Adjust path as necessary
